...
func TestSubMenu_ToEvent(t *testing.T) {
	// App sub menu
	var s = newSubMenu(nil, nil, []*MenuItemOptions{{Label: PtrStr("1")}, {Label: PtrStr("2")}}, asticontext.NewCanceller(), nil, newIdentifier(), nil)
	e := s.toEvent()
	...
	s = newSubMenu(nil, w, []*MenuItemOptions{{Label: PtrStr("1")}, {Label: PtrStr("2")}}, asticontext.NewCanceller(), nil, i, nil)
	e = s.toEvent()
...
func TestSubMenu_SubMenu(t *testing.T) {
	...
	var m = newMenu(context.Background(), nil, o, nil, nil, newIdentifier(), nil)
	s, err := m.SubMenu(0, 1)
