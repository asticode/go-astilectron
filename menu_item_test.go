...
func TestMenuItem_ToEvent(t *testing.T) {
	...
	var mi = newMenuItem(context.Background(), nil, o, nil, nil, newIdentifier(), nil)
	e := mi.toEvent()
...
func TestMenuItem_Actions(t *testing.T) {
	...
	var w = newWriter(wrt)
	var mi = newMenuItem(context.Background(), nil, &MenuItemOptions{Label: PtrStr("label")}, c, d, i, w)
