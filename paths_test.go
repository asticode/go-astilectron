...
func TestPaths(t *testing.T) {
	...
	os.Setenv(k, pad)
	p, err = newPaths("windows", "amd64", Options{AppName: "Test app"})
	assert.NoError(t, err)
	assert.Equal(t, pad+"/Test app", p.DataDirectory())
	assert.Equal(t, pad+"/Test app/vendor", p.VendorDirectory())
	assert.Equal(t, pad+"/Test app/vendor/electron-windows-amd64/Test app.exe", p.AppExecutable())
	...
	assert.Equal(t, pad+"/Test app/vendor/electron-windows-amd64-v"+VersionElectron+".zip", p.ElectronDownloadDst())
	assert.Equal(t, pad+"/Test app/vendor/electron-windows-amd64-v"+VersionElectron+".zip", p.ElectronUnzipSrc())
